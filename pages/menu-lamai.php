...
// Photos from DB
$photos_stmt = $pdo->prepare("SELECT * FROM menu_images WHERE branch='lamai' ORDER BY sort_order ASC");
$photos_stmt->execute();
...
.photo-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:40px;}
...
.photo-grid img{width:100%;aspect-ratio:4/3;object-fit:cover;display:block;cursor:pointer;transition:transform .3s,opacity .3s;}
.photo-grid img:hover{transform:scale(1.02);opacity:.85;}
@media(max-width:540px){.photo-grid img{aspect-ratio:unset;object-fit:unset;height:auto;}}
...
<section style="padding:90px 24px 40px;text-align:center;background:#0A0A0A;">
    <p style="font-size:.65rem;letter-spacing:.35em;color:#B8860B;margin-bottom:14px;font-family:'Inter',sans-serif;text-transform:uppercase;">ROYAL TURKISH CUISINE</p>
    <h1 style="font-family:'Playfair Display',serif;font-size:clamp(2.2rem,5vw,3.8rem);color:white;margin:0 0 18px 0;">Lamai Menu</h1>
    <div class="gold-line"></div>
    <p style="color:#52525b;font-size:.82rem;margin-top:20px;letter-spacing:.05em;"><?= LAMAI_ADDRESS ?></p>
    <div class="menu-tabs">
        <a href="<?= htmlspecialchars($chaweng_url) ?>" class="menu-tab">CHAWENG</a>
        <a href="<?= htmlspecialchars($lamai_url) ?>" class="menu-tab active">LAMAI</a>
    </div>
</section>
